...
public class loadDat {
    ...
    public static boolean foundEOF;
    ...
    public final boolean readWorkbookGlobals() {
        try {
            objectType = 0;
            foundEOF = false;
            pos = 0;
    ...
    public final void parseObject(int type) {
        try {
            switch (type) {
                case 0:
                    //found BOF
                ...
                case 14:
                    //found string object
                    len = byteToInt(2);
                    ResourceContainer.strings[ltmp] = new String(chapter, pos, len, "UTF-8");
                    pos += len;
                    ltmp++;
                    return;
                ...
                case 29:
                    ...
                    if (ResourceContainer.flags == null) {
                        loadFlags = true;
                        ResourceContainer.flags = new boolean[k];
                ...
                case 40:
                    //found string object
                    len = byteToInt(2);
                    ResourceContainer.consoleStrings[ltmp] = new String(chapter, pos, len, "UTF-8");
                    pos += len;
                    ltmp++;
                    return;
    ...
    public final boolean convertToBool(int i) {
        if (i == 0) {
            return false;
        } else {
            return true;
    ...
    public final int byteToInt(int num) {
        switch (num) {
            case 1:
                pos++;
    ...
    public final int processInt(int src) {
        if (src > 60000) {
    ...
    public static final int byteShortToInt(byte[] a, int pos) {
        int i = 0;
        i |= (a[(pos + 1)] & 0xFF);
        i <<= 8;
        i |= (a[pos] & 0xFF);
    ...
    // NOTE - this function uses little-endian format
    public static final int byteIntToInt(byte[] a, int pos) {
        int i = 0;
        i |= (a[(pos + 3)] & 0xFF);
        i <<= 8;
        i |= (a[(pos + 2)] & 0xFF);
