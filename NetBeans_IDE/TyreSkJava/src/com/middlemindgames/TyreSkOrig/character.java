 ...
 */
public class character extends charCore {
    public Random rand;
    public int enm;
    public int ex;
    public int charType;
    ...
    public boolean canSelectAny = false;
    public boolean canSelectAll = false;

    //battle affliction protections
    public boolean antisleep = false;
    public boolean antistun = false;
    public boolean antipoison = false;

    //track the orb separately since herbal antisleep overwrites the variable
    public boolean antisleeporb = false;
    public int weapon;
    public int armor;
    public int tgold;
    ...
    public character() {
        ...
        items = new int[Constants.ITEM_INVENTORY_SIZE][2];
    ...
    public void startBattle(int enm) {
        ...
        for (i = 0; i < enm; i++) {
            k = rand.nextInt(ResourceContainer.linkTables[Tyre.currentRoomIdx].enemyLevel);
            j = rand.nextInt(ResourceContainer.enemies[k].length);
            ...
            if (MainWindow.enemies[i].goldCoinsMax != 0) {
                tgold = rand.nextInt(MainWindow.enemies[i].goldCoinsMax);
                tgold++;
                if (tgold < 3) {
                    MainWindow.gold += 3;
                } else {
