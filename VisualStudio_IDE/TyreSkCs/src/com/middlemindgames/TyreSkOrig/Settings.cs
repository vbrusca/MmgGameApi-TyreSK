...
namespace com.middlemindgames.TyreSkOrig {
    public class Settings {
        static private bool REQUEST_AUTO_SYNC = true;
        ...
        private static int gameStart;  //byte
        ...
        private Settings() {
            ...
            battleModeState = -1;
            playerName = "";
            items = MainWindow.NewMdIntArray(50, 2, -1); //new byte[50][]; //byte[50][];
            
            /*
            for (i = 0; i < items.Length; i++) {
                items[i] = new int[] { -1, -1 }; //new byte[] { -1, -1 };
            }
            */
        }
        ...
        public byte[] getSerializedInvData() {
            try {
                ...
                Array.Copy(tmpBytes, 0, serializedInvData, 0, tmpBytes.Length);
        ...
        public byte[] getSerializedGameData() {
            try {
                ...
                Array.Copy(tmpBytes, 0, serializedGameData, 0, tmpBytes.Length);
        ...
        public byte[] getSerializedStateData() {
            try {
                ...
                Array.Copy(tmpBytes, 0, serializedStateData, 0, tmpBytes.Length);
            } catch (SettingsDBException ex) {
                //ex.printStackTrace();            
            }
        ...
        public bool[] getFlags(bool[] currentFlags) {
            try {
                tmpBytes = sSettingsDB.getBytes("flgs");
                if (tmpBytes != null) {
                    if (currentFlags.Length > tmpBytes.Length) {
                        flagsOut = new bool[currentFlags.Length];
                        ...
                        flagsOut = new bool[tmpBytes.Length];
                        for (i = 0; i < flagsOut.Length; i++) {
                            if (tmpBytes[i] == (byte)1) {
                                flagsOut[i] = true;
                            } else {
        ...
        public void setFlags(bool[] newFlags) {
            tmpBool = false;
            if (newFlags != null) {
