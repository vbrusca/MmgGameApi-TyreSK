...
namespace com.middlemindgames.TyreSkOrig {
    ...
    public class Tyre : Application {
        ...
        public static bool[] adjBuildFlag;
        ...
        public static bool firstContinue;
        public static bool hasLink;
        public static bool hasCanWalkDynamic;
        ...
        public Tyre() {
            ...
            MmgFont.PASSIVE_FONT_SIZE = true;

            try {
                speedMultiplier = Double.Parse(Tyre.cApp.getString(Resources.SPEED_MULTIPLIER).Trim());
            } catch (FormatException nfe) {
                speedMultiplier = 1.0;
            }

            try {
                defaultSpeed = Int32.Parse(Tyre.cApp.getString(Resources.PLAYER_SPEED).Trim());
            } catch (FormatException nfe) {
                defaultSpeed = 4;
            }
        }

        public override bool receiveEvent(Event e) {
            consumed = false;
            switch (e.type) {
        ...
        public void suspend() {
            Logger.wr("Tyre: suspend: AAA");
            suspended = true;
            Logger.wr("Tyre: suspend: BBB: 1");
            if (MainWindow.gameState == 8) {
                Logger.wr("Tyre: suspend: BBB: 2");
                if (MainWindow.datKilled == false) {
                    MainWindow.datKilled = true;
                    killDatLoad();
                }
                Logger.wr("Tyre: suspend: BBB: 3");
            } else {
                Logger.wr("Tyre: suspend: BBB: 4");
                if (!gameStart) {
                    try {
                        storeSettings();
                ...
                Logger.wr("Tyre: suspend: BBB: 5");
            }

            Logger.wr("Tyre: suspend: BBB: 6");
            if (mTimer != null) {
                mTimer.stop();
                cancelEvents(Event.EVENT_TIMER, Event.EVENT_TIMER);
            }

            Logger.wr("Tyre: suspend: BBB: 7");
            TyreAudio.cleanup();
        ...
        */
        ...
        private void loadCompressedDatFile() {
            ...
            try {
                //decompresser.
                resultLength = decompresser.inflate(out result, 0, result.Length);
                decompresser.end();
        ...
        */
        ...
        public void xTrdHandleConsoleBitmaps() {
            try {
        ...
        public void xTrdHandleSequences() {
            try {
                ...
                for (i = 0; i < ResourceContainer.sequences.Length; i++) {
                    ...
                    if (sequences[i] != null) {
                        ...
                        if (i == 0) {
                            if (sequences[i].loadFromFile(MmgHelper.GetBasicCachedSound("tyre-battle1-short-version-sm.wav")) == false) {
                            } else {
                            ...
                            if (sequences[i].loadFromFile(MmgHelper.GetBasicCachedSound("tyre-bazaar-edited-short-version.wav")) == false) {
                            } else {
                                sequences[i].start(ToneFilter.NONE);
                                //music...
                                sequences[i].setLoops(10000);
                            ...
                            if (sequences[i].loadFromFile(MmgHelper.GetBasicCachedSound("tyre-wilderness.wav")) == false) {
                            } else {
                                sequences[i].start(ToneFilter.NONE);
                                //music...
                                sequences[i].setLoops(10000);
                            ...
                            if (sequences[i].loadFromFile(MmgHelper.GetBasicCachedSound("tyre-title-short-version.wav")) == false) {
                            } else {
                                sequences[i].start(ToneFilter.NONE);
                                //music...
                                sequences[i].setLoops(10000);
                            ...
                            if (sequences[i].loadFromFile(MmgHelper.GetBasicCachedSound("tyre-dungeon-short-version.wav")) == false) {
                            } else {
                                sequences[i].start(ToneFilter.NONE);
                                //music...
                                sequences[i].setLoops(10000);
                            ...
                            if (sequences[i].loadFromFile(MmgHelper.GetBasicCachedSound("tyre-battle2-short-version-sm.wav")) == false) {
                            } else {
                                sequences[i].start(ToneFilter.NONE);
                                //music...
                                sequences[i].setLoops(10000);
        ...
        public void xTrdHandlePrepConsoleBitmaps() {
            try {
                setSizeInfo();
                //check for kill load
                if (loadDat.foundEOF) {
                ...
                pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget((RenderTarget2D)consoleBitmaps[MainWindow.GAMEHUD_INDEX].mmgBmp.GetTexture2D());
                ...
                pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget((RenderTarget2D)consoleBitmaps[MainWindow.BATTLEHUD_INDEX].mmgBmp.GetTexture2D());
                pen.mmgPen.GetGraphics().Begin(SpriteSortMode.Immediate, BlendState.NonPremultiplied);
                ...
                //System.out.println(" KILLDAT " + MainWindow.datKilled);
                if (MainWindow.datKilled) {
                    ...
                    //System.out.println("Setting state back to menu from  "+ MainWindow.gameState + ":" +MainWindow.gameSubState);
        ...
        public void finalizeLoad() {
            ...
            //ask for strings
            if (useXtrd == true) {
                datLoader.xTrdW.AddCommand("xTrdHandleStrings", null);
                while (datLoader.xTrdW.commands.Count > 0) { }
            } else {
                for (int z = 0; z < ResourceContainer.consoleStrings.Length; z++) {
                    ResourceContainer.consoleStrings[z] = sanitizeSkChars(ResourceContainer.consoleStrings[z]);
                    if (ResourceContainer.consoleStrings[z].Contains("adjunct")) {
            ...
            //ask for bitmaps
            if (useXtrd == true) {
                datLoader.xTrdW.AddCommand("xTrdHandleBitmaps", null);
                while (datLoader.xTrdW.commands.Count > 0) { }
            } else {
                try {
                    // check for kill load
            ...
            //ask for console bitmaps
            if (useXtrd == true) {
                datLoader.xTrdW.AddCommand("xTrdHandleConsoleBitmaps", null);
                while (datLoader.xTrdW.commands.Count > 0) { }
            } else {
                try {
                    consoleBitmaps = new Bitmap[ResourceContainer.consoleBitmaps.Length + MainWindow.CONSOLEBITMAPS_NUM_DYNAMIC_IMAGES];
            ...
            //ask for sequences
            if (useXtrd == true) {
                datLoader.xTrdW.AddCommand("xTrdHandleSequences", null);
                while (datLoader.xTrdW.commands.Count > 0) { }
            } else {
                try {
                    sequences = new Sequence[ResourceContainer.sequences.Length];
            ...
            //ask to prep console bitmaps
            if (useXtrd == true) {
                datLoader.xTrdW.AddCommand("xTrdHandlePrepConsoleBitmaps", null);
                while (datLoader.xTrdW.commands.Count > 0) { }
            } else {
                try {
                    setSizeInfo();
        ...
        private void buildAdjascentRoom(bool sort, int index, int z) {
            if (z != -1) {
                ...
                Pen pen = adjRoom[index].createPen();
                pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget((RenderTarget2D)adjRoom[index].mmgBmp.GetTexture2D());
                pen.mmgPen.GetGraphics().Begin(SpriteSortMode.Immediate, BlendState.NonPremultiplied);
                
                mWindow.drawLayerOneFlaggedObjects(pen, false);
                pen.mmgPen.GetGraphics().End();
                pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget(null);
            } else {
                adjRoom[index] = new Bitmap(MainWindow.ROOM_WIDTH, MainWindow.ROOM_HEIGHT);
        ...
        public void buildAdjascentRooms(bool sort) {
            //right
        ...
        public void buildRoom(bool sort) {
        ...
        public void buildRoom(bool sort, int roomId, Bitmap roomImg, Bitmap layer2Img, bool activeRoom) {
            ...
            roomImg.pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget((RenderTarget2D)roomImg.mmgBmp.GetTexture2D());
            roomImg.pen.mmgPen.GetGraphics().Begin(SpriteSortMode.Immediate, BlendState.NonPremultiplied);
            ...
            for (z = 0; z < roomDataInfo.Length; z++) {
                ...
                for (j = rDat.top; j < rDat.bottom; j += TILESIZE) {
                    for (i = rDat.left; i < rDat.right; i += TILESIZE) {
                        ...
                        Bitmap.copyBits(roomImg.pen, roomImg, new Rect(i, j, i + TILESIZE, j + TILESIZE), bitmaps[ResourceContainer.tiles[rDat.tiles_id].imageIndex], new Rect(TILESIZE, TILESIZE));
                    }
                }
            }
            ...
            roomImg.pen.mmgPen.GetGraphics().End();
            roomImg.pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget(null);

            layer2Img.pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget((RenderTarget2D)layer2Img.mmgBmp.GetTexture2D());
            layer2Img.pen.mmgPen.GetGraphics().Begin(SpriteSortMode.Immediate, BlendState.NonPremultiplied);
            layer2Img.pen.mmgPen.GetGraphics().GraphicsDevice.Clear(MmgColor.GetTransparent().GetColor());
            ...
            layer2Img.pen.mmgPen.GetGraphics().End();
            layer2Img.pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget(null);
        ...
        private void drawLayerOne(int tmp, int angle, int xOffset, int yOffset, Bitmap roomImg) {
            ...
            //changing 240, 136 to MainWindow.ROOM_WIDTH, MainWindow.ROOM_HEIGHT
            ...
            if (angle == 0) {
                try {
                    Bitmap.copyBits(roomImg.pen, roomImg, new Rect(x, y, xw, yh), bitmaps[ResourceContainer.objects[tmp].imageIndex], new Rect(0, 0, w, h));
                } catch (Exception e) {
                    Logger.wr(e.Message);
                    Logger.wr(e.StackTrace);
                }
                ...
                try {
                    Bitmap.copyBits(roomImg.pen, roomImg, new Rect(x, y, xw, yh), bitmaps[ResourceContainer.objects[tmp].imageIndex].rotateBy(angle), new Rect(0, 0, w, h));
                } catch (Exception e) {
                    Logger.wr(e.Message);
                    Logger.wr(e.StackTrace);
                }
        ...
        public Bitmap drawConsoleWindow(int width, int height, int trans) {
            ...
            pen.mmgPen.GetGraphics().GraphicsDevice.SetRenderTarget((RenderTarget2D)window.mmgBmp.GetTexture2D());
            pen.mmgPen.GetGraphics().Begin(SpriteSortMode.Immediate, BlendState.NonPremultiplied);
        ...
        public void drawConsoleWindow(Pen pen, int width, int height, int xoff, int yoff) {
        ...
        public Bitmap drawTarget(int width, int height) {
            return drawTarget(width, height, 9, 2, Color.RED);
        }

        public Bitmap drawTarget(int width, int height, int blength, int bheight, int color) {
            Bitmap window = new Bitmap(width, height);
            pen = window.createPen();
        ...
        public void restoreSettings() {
            ...
            bool[] flgTmp = theSettings.getFlags(ResourceContainer.flags);
            if (flgTmp != null) {
        ...
        public static void PutBooleanDataIntoBytes(byte[] a, int i, bool d) {
            if (d) {
        ...
        public static bool GetBooleanDataFromBytes(byte[] a, int i) {
